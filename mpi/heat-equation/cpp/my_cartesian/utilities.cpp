...
double average(const Field& field, const ParallelData parallel)
{
     ...
     local_average /= (field.nx * field.ny);

     MPI_Barrier(parallel.communicator);
     MPI_Allreduce(&local_average, &average, 1, MPI_DOUBLE, MPI_SUM, parallel.communicator);
     ...
     //         MPI_Recv(&local_average, 1, MPI_DOUBLE, p, 22, parallel.communicator, MPI_STATUS_IGNORE);
     //         average += local_average;
     //     }
     //     average /= (field.nx_full * field.ny_full);
     // } else {
     //     MPI_Send(&local_average, 1, MPI_DOUBLE, 0, 22, parallel.communicator);
     // }
