...
struct ParallelData {
    ...
    ParallelData() {      // Constructor
      ...
      MPI_Cart_create(MPI_COMM_WORLD, ndims, dims, periods, true, &communicator);
      MPI_Comm_rank(communicator, &rank);
      MPI_Cart_shift(communicator, 0,  1, &nup, &ndown);

      // nup = rank-1;
      // ndown = rank+1;
      //
      // if (nup < 0) nup = MPI_PROC_NULL;
      // if (ndown > size - 1) ndown = MPI_PROC_NULL;
