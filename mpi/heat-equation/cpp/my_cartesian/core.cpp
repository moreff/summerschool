...
void exchange_init(Field& field, ParallelData& parallel) {
    ...
    MPI_Irecv(rbuf_down, field.ny+2, MPI_DOUBLE, parallel.ndown, 12, parallel.communicator, &parallel.requests[0]);
    MPI_Isend(sbuf_up,   field.ny+2, MPI_DOUBLE, parallel.nup,   12, parallel.communicator, &parallel.requests[1]);

    // Send to down, receive from up
    double* sbuf_down = field.temperature.data(field.nx, 0);
    double* rbuf_up   = field.temperature.data();
    MPI_Irecv(rbuf_up,   field.ny+2, MPI_DOUBLE, parallel.nup,   12, parallel.communicator, &parallel.requests[2]);
    MPI_Isend(sbuf_down, field.ny+2, MPI_DOUBLE, parallel.ndown, 12, parallel.communicator, &parallel.requests[3]);
}
