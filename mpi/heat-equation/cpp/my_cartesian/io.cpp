    ...
    if (0 == parallel.rank) {
        ...
        for (int p = 1; p < parallel.size; p++) {
            MPI_Recv(tmp_mat.data(), field.nx * field.ny,
                     MPI_DOUBLE, p, 22, parallel.communicator, MPI_STATUS_IGNORE);
        ...
        MPI_Send(tmp_mat.data(), field.nx * field.ny,
                 MPI_DOUBLE, 0, 22, parallel.communicator);
    }
                ...
                const ParallelData parallel)
{
    ...
    std::cout << filename << ": " << nx_full << "x" << ny_full << comment << std::endl;
    ...
    if (0 == parallel.rank) {
        ...
        //              MPI_DOUBLE, p, 22, parallel.communicator);
        // }
    } //else {
    //     MPI_Recv(inner.data(), field.nx * field.ny,
    //              MPI_DOUBLE, 0, 22, parallel.communicator, MPI_STATUS_IGNORE);
    // }
    file.close();

    MPI_Scatter(full.data(),  field.nx * field.ny, MPI_DOUBLE,
                inner.data(), field.nx * field.ny, MPI_DOUBLE,
                0, parallel.communicator);
    MPI_Barrier(parallel.communicator);
