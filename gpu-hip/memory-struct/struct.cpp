...
Example* createDeviceExample(Example *ex)
{
  ...
  hipMemcpy(d_idx, ex->idx, ex->size*sizeof(float), hipMemcpyHostToDevice);
...
void freeDeviceExample(Example *d_ex)
{
  ...
  hipMemcpy(&d_idx, &(d_ex->idx), sizeof(int*), hipMemcpyHostToDevice);
...
void runDeviceExplicitMem()
{
  ...
  Example *ex;
  hipHostMalloc((void **) &ex, sizeof(Example));
  ex->size = 10;

  // #error Allocate host struct members
  hipHostMalloc((void**) &(ex->x), ex->size*sizeof(float));
  hipHostMalloc((void**) &(ex->idx), ex->size*sizeof(int));
